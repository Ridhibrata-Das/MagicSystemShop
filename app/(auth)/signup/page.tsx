...
import { signup, loginWithGoogle, subscribeToAuth } from "@/services/authService";
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate
    const result = signupSchema.safeParse({ name: cleanName, email: cleanEmail, password });
    if (!result.success) {
    ...
    const { user, error: authError } = await signup(result.data);
    if (authError) {
      setError(authError);
      setLoading(false);
      return;
  ...
  return (
    <div className="w-full max-w-md mx-auto flex-grow flex items-center justify-center animate-fade-in-up py-12 px-4 sm:px-6 lg:px-8">
      <SystemWindow title="SYSTEM REGISTRATION" className="w-full">
        <h3 className="mb-6 text-center text-sm font-orbitron font-bold text-system-muted tracking-[0.2em] uppercase">Initialize New Entity</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-xs font-orbitron font-bold text-system-accent uppercase tracking-widest mb-1">Entity Designation (Name)</label>
            <div className="mt-1">
              <input
                type="text"
                required
          ...
          <div>
            <label className="block text-xs font-orbitron font-bold text-system-accent uppercase tracking-widest mb-1">Comm Link (Email)</label>
            <div className="mt-1">
              <input
                type="email"
                required
        ...
        <div className="mt-6 pt-4 text-center text-sm font-rajdhani">
          <span className="text-system-muted">Already Registered? </span>
          <Link href="/login" className="font-orbitron font-bold text-system-accent hover:text-white transition-colors">
            Initiate Login
          </Link>
        </div>
      </SystemWindow>
