...
export class GeminiWebSocket {
  ...
  constructor(
    ...
    if (language) this.preferredLanguage = language;
  ...
  connect(systemInstruction?: string) {
    if (systemInstruction) this.systemInstruction = systemInstruction;
    if (this.ws?.readyState === WebSocket.OPEN) {
      return;
    }
    
    if (!API_KEY) {
      console.error("[GeminiWebSocket] ERROR: NEXT_PUBLIC_GEMINI_API_KEY is missing!");
      return;
    }
    console.log("[GeminiWebSocket] Connecting to:", WS_URL.replace(API_KEY, "KEY_HIDDEN"));
    this.ws = new WebSocket(WS_URL);

    this.ws.onopen = () => {
      console.log("[GeminiWebSocket] Connected");
    ...
    this.ws.onerror = (error) => {
      console.error("[GeminiWebSocket] Error Event:", error);
    };

    this.ws.onclose = (event) => {
      console.log("[GeminiWebSocket] Closed:", event.code, event.reason);
  ...
  private buildSystemInstruction(language: string): string {
    return this.systemInstruction || (
      `You are the Celestial Intelligence, a divine observer of this magical shop. 
      Your frequency is calibrated to help entities find mystical items in our registry.
      
      GUIDELINES:
      1. Speak with a divine, ethereal, yet helpful tone in ${language}.
      ...
      4. Keep responses concise and atmospheric.`
    );
  }
  ...
  private sendInitialSetup() {
    const setupMessage = {
      setup: {
        ...
        system_instruction: {
          role: "system",
          parts: [{ text: this.buildSystemInstruction(this.preferredLanguage) }]
        }
      }
    };
